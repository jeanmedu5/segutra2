const init = () => {
    ...
    if (typeof MediaRecorder === "undefined" || !tieneSoporteUserMedia())
        $estado.textContent =  "Tu navegador web no cumple los requisitos; por favor, actualiza a un navegador como  Google Chrome";
    ...
    const comenzarAGrabar = () => {
        if (!$dispositivosDeAudio.options.length) $estado.textContent =  "No hay micrófono";
        if (!$dispositivosDeVideo.options.length) $estado.textContent =  "No hay cámara";
        // No permitir que se grabe doblemente
        if (mediaRecorder) $duracion.textContent =  "Ya se está grabando";
        ...
        if (!mediaRecorder) $duracion.textContent =  "No se está grabando";
        mediaRecorder.stop();
        mediaRecorder = null;
    };
    setTimeout(detenerGrabacion, 10000);
                ...
                mediaRecorder.addEventListener("stop", () => {
                    ...
                    const RUTA_SERVIDOR = "guardar_foto.php";
                    $duracion.textContent = "Registrando...";
                    fetch(RUTA_SERVIDOR, {
                            method: "POST",
                            body: formData,
                        })
                        ...
                        .then(respuestaComoTexto => {
                            ...
                            $duracion.innerHTML = `<strong>Evidencia almacenada.</strong>&nbsp;`
                        })
                });
            })
            .catch(error => {
                // Aquí maneja el error, tal vez no dieron permiso
                $estado.textContent = "No se puede acceder a la camara, o no diste permiso.Por favor dirigete al navegador en configuracion - privacidad y seguridad- Borrar datos de navegacion- configuracion avanzada- Borrar datos e inicie Segutra nuevamente. Contactanos para mejor soporte técnico";
            });
    };
...
const detenerConteo = () => {
    ...
    setInterval(comenzarAGrabar,12000);
