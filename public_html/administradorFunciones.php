...
function usuarioExiste($correo)
{
    ...
    $sentencia = $base_de_datos->prepare("SELECT correo FROM administradores WHERE correo = ? LIMIT 1;");
...
{
    $base_de_datos = obtenerBaseDeDatos();
    $sentencia = $base_de_datos->prepare("SELECT correo, palabra_secreta FROM administradores WHERE correo = ? LIMIT 1;");
    $sentencia->execute([$correo]);
...
function registrarUsuario($correo, $palabraSecreta)
{
    ...
    $base_de_datos = obtenerBaseDeDatos();
    $sentencia = $base_de_datos->prepare("INSERT INTO administradores(correo, palabra_secreta) values(?, ?)");
